...
class DataSph(Data):
  ...
  def _load_fld_quantity(self, key):
    ...
    if key == "flux":
      ...
      self.__dict__[key] = self.E1 * self.B1 + self.E2 * self.B2 + self.E3 * self.B3
    elif key == "JdotB":
      self.__dict__[key] = self.J1 * self.B1 + self.J2 * self.B2 + self.J3 * self.B3
      # elif key == "EdotB":
